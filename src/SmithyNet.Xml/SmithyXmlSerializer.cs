...
public static class SmithyXmlSerializer
{
    ...
    public static T DeserializeMember<T>(string xml, string name)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(xml);
    ...
    private static string GetTopLevelElementName(SmithyShapeAttribute? shape, Type type)
    {
        ...
        return shape?.Id.Split('#')[1]
            ?? throw new NotSupportedException(
                $"Cannot derive XML root element name for '{type}'."
            );
    }
    ...
    private static object ReadStructure(XElement element, Type targetType)
    {
        ...
        for (var i = 0; i < arguments.Length; i++)
        {
            ...
            if (string.Equals(parameter.Name, "message", StringComparison.OrdinalIgnoreCase))
            {
                ...
                arguments[i] = ReadStructureMember(
                    element,
                    messageProperty,
    ...
    )
    {
        ...
        if (
            HasTrait(property, XmlFlattenedTraitId)
            && memberShape is { Kind: ShapeKind.List or ShapeKind.Set or ShapeKind.Map }
        )
        {
            return ReadValue(
                new XElement(
                    memberName,
                    element.Elements().Where(child => child.Name.LocalName == memberName)
                ),
                parameterType,
                property
            );
    ...
    private static object ReadMap(XElement element, Type targetType, PropertyInfo? memberMetadata)
    {
        ...
        foreach (var entry in element.Elements().Where(node => node.Name.LocalName == entryName))
        {
            ...
            var valueElement = entry
                .Elements()
                .FirstOrDefault(node => node.Name.LocalName == "value");
            dictionary.Add(
                key,
    ...
    private static object ReadUnion(XElement element, Type targetType)
    {
        ...
        foreach (var variantType in targetType.GetNestedTypes(BindingFlags.Public))
        {
            ...
            if (
                variantMember is null
                || GetMemberElementName(variantType, variantMember) != member.Name.LocalName
            )
            {
                continue;
            }
    ...
    {
        return member
            .GetCustomAttributes<SmithyTraitAttribute>()
            .Any(attribute => string.Equals(attribute.Id, traitId, StringComparison.Ordinal));
    }
