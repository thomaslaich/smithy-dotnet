...
NSmithy can generate documentation UIs from your Smithy model and serve them
alongside your ASP.NET Core application:

- **Scalar (OpenAPI)** — an interactive OpenAPI explorer at `/openapi`, backed
  by a `openapi.json` generated from the model by
...
- **Sphinx HTML** — auto-generated reference documentation at `/docs`, produced
  ...
  to HTML automatically at build time. Available for HTTP services (see the
  note on bote services below).
- **Scalar (AsyncAPI)** — an interactive AsyncAPI 3.1 reference at `/asyncapi`,
  backed by an `asyncapi.json` generated by bote's smithy-asyncapi plugin.
  Available for [bote messaging services](/smithy-dotnet/protocols/bote-overview/)
  (e.g. `@kafkaJson`).

All are opt-in MSBuild features. Their output is copied into `wwwroot/`, where
...
```xml
<PropertyGroup>
  <!-- Generate Sphinx HTML documentation (HTTP services) -->
...
app.MapSmithyAsyncApi(); // mounts Scalar at /asyncapi (requires SmithyGenerateAsyncApi)
app.MapMyService();
app.Run();
```
...
`MapSmithyAsyncApi()` enables static file serving and mounts Scalar at
...
`SmithyGenerateDocs` therefore cannot be enabled for bote messaging services:
