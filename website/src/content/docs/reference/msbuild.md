...
| `SmithyGenerateDependencyInjection` | `false` | Compile the generated `Add{Service}Client` IHttpClientFactory extension. Requires referencing `Microsoft.Extensions.Http` (or the `Microsoft.AspNetCore.App` shared framework). See [Dependency Injection](/smithy-dotnet/guides/dependency-injection/). |
