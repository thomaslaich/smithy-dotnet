...
| `SmithyGenerateFakes` | `false` | Generate `Fake{Service}Handler` and the `AddFake{Service}Handler` registration extension (flows into `smithy-build.json` as the `csharp-codegen` `generateFakes` setting). The fake returns each operation's first non-error `@examples` output when present, otherwise deterministic placeholder values synthesized from the model. Requires `SmithyGenerateServer`. |
